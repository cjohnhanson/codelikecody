...
fn print_with_boundaries(content: &str, origin: Option<&str>, opts: &OutputOptions) {
    ...
    if opts.content_boundaries {
        ...
        println!(
            "--- MOOSE_PAGE_CONTENT nonce={} origin={} ---",
            nonce, origin_str
        );
        println!("{}", content);
        println!("--- END_MOOSE_PAGE_CONTENT nonce={} ---", nonce);
    } else {
        println!("{}", content);
    }
}
...
pub fn print_response_with_opts(resp: &Response, action: Option<&str>, opts: &OutputOptions) {
    ...
    if let Some(data) = &resp.data {
            ...
            .unwrap_or(false)
        {
            ...
            println!("  Run: moose confirm {}", cid);
            println!("  Or:  moose deny {}", cid);
            return;
        }
        if data
            .get("confirmed")
...
pub fn print_command_help(command: &str) -> bool {
    let help = match command {
        ...
        "open" | "goto" | "navigate" => {
            r##"
moose open - Navigate to a URL

Usage: moose open <url>
...
Examples:
  moose open example.com
  moose open https://github.com
  moose open localhost:3000
  moose open api.example.com --headers '{"Authorization": "Bearer token"}'
    # ^ Headers only sent to api.example.com, not other domains
"##
        }
        "back" => {
            r##"
moose back - Navigate back in history
...
"##
        ...
        "forward" => {
            r##"
moose forward - Navigate forward in history
...
"##
        ...
        "reload" => {
            r##"
moose reload - Reload the current page
...
"##
        ...
        "click" => {
            r##"
moose click - Click an element

Usage: moose click <selector> [--new-tab]
...
Examples:
  moose click "#submit-button"
  moose click @e1
  moose click "button.primary"
  moose click "//button[@type='submit']"
  moose click @e3 --new-tab
"##
        }
        "dblclick" => {
            r##"
moose dblclick - Double-click an element

Usage: moose dblclick <selector>
...
Examples:
  moose dblclick "#editable-text"
  moose dblclick @e5
"##
        }
        "fill" => {
            r##"
moose fill - Clear and fill an input field

Usage: moose fill <selector> <text>
...
Examples:
  moose fill "#email" "user@example.com"
  moose fill @e3 "Hello World"
  moose fill "input[name='search']" "query"
"##
        }
        "type" => {
            r##"
moose type - Type text into an element

Usage: moose type <selector> <text>
...
Examples:
  moose type "#search" "hello"
  moose type @e2 "additional text"

See Also:
  For typing into contenteditable editors (Lexical, ProseMirror, etc.)
  without a selector, use 'keyboard type' instead:
    moose keyboard type "# My Heading"
"##
        }
        "hover" => {
            r##"
moose hover - Hover over an element

Usage: moose hover <selector>
...
Examples:
  moose hover "#dropdown-trigger"
  moose hover @e4
"##
        }
        "focus" => {
            r##"
moose focus - Focus an element

Usage: moose focus <selector>
...
Examples:
  moose focus "#input-field"
  moose focus @e2
"##
        }
        "check" => {
            r##"
moose check - Check a checkbox

Usage: moose check <selector>
...
Examples:
  moose check "#terms-checkbox"
  moose check @e7
"##
        }
        "uncheck" => {
            r##"
moose uncheck - Uncheck a checkbox

Usage: moose uncheck <selector>
...
Examples:
  moose uncheck "#newsletter-opt-in"
  moose uncheck @e8
"##
        }
        "select" => {
            r##"
moose select - Select a dropdown option

Usage: moose select <selector> <value...>
...
Examples:
  moose select "#country" "US"
  moose select @e5 "option2"
  moose select "#menu" "opt1" "opt2" "opt3"
"##
        }
        "drag" => {
            r##"
moose drag - Drag and drop

Usage: moose drag <source> <target>
...
Examples:
  moose drag "#draggable" "#drop-zone"
  moose drag @e1 @e2
"##
        }
        "upload" => {
            r##"
moose upload - Upload files

Usage: moose upload <selector> <files...>
...
Examples:
  moose upload "#file-input" ./document.pdf
  moose upload @e3 ./image1.png ./image2.png
"##
        }
        "download" => {
            r##"
moose download - Download a file by clicking an element

Usage: moose download <selector> <path>
...
Examples:
  moose download "#download-btn" ./file.pdf
  moose download @e5 ./report.xlsx
  moose download "a[href$='.zip']" ./archive.zip
"##
        }
        ...
        "press" | "key" => {
            r##"
moose press - Press a key or key combination

Usage: moose press <key>
...
Examples:
  moose press Enter
  moose press Tab
  moose press Control+a
  moose press Control+Shift+s
  moose press Escape
"##
        }
        "keydown" => {
            r##"
moose keydown - Press a key down (without release)

Usage: moose keydown <key>
...
Examples:
  moose keydown Shift
  moose keydown Control
"##
        }
        "keyup" => {
            r##"
moose keyup - Release a key

Usage: moose keyup <key>
...
"##
        ...
        "keyboard" => {
            r##"
moose keyboard - Raw keyboard input (no selector needed)

Usage: moose keyboard <subcommand> <text>
...
Examples:
  moose keyboard type "Hello, World!"
  moose keyboard type "# My Heading"
  moose keyboard inserttext "pasted content"

Use Cases:
  # Type into a Lexical/ProseMirror contenteditable editor:
  moose click "[contenteditable]"
  moose keyboard type "# My Heading"
  moose press Enter
  moose keyboard type "Some paragraph text"
"##
        }
        ...
        "scroll" => {
            r##"
moose scroll - Scroll the page

Usage: moose scroll [direction] [amount] [options]
...
Examples:
  moose scroll
  moose scroll down 500
  moose scroll up 200
  moose scroll left 100
  moose scroll down 500 --selector "div.scroll-container"
"##
        }
        "scrollintoview" | "scrollinto" => {
            r##"
moose scrollintoview - Scroll element into view

Usage: moose scrollintoview <selector>
...
Examples:
  moose scrollintoview "#footer"
  moose scrollintoview @e15
"##
        }
        ...
        "wait" => {
            r##"
moose wait - Wait for condition

Usage: moose wait <selector|ms|option>
...
Examples:
  moose wait "#loading-spinner"
  moose wait 2000
  moose wait --url "**/dashboard"
  moose wait --load networkidle
  moose wait --fn "window.appReady === true"
  moose wait --text "Welcome back"
  moose wait --download ./file.pdf
  moose wait --download ./report.xlsx --timeout 30000
  moose wait --fn "!document.body.innerText.includes('Loading...')"
"##
        }
        ...
        "screenshot" => {
            r##"
moose screenshot - Take a screenshot

Usage: moose screenshot [selector] [path]
...
Examples:
  moose screenshot
  moose screenshot ./screenshot.png
  moose screenshot --full ./full-page.png
  moose screenshot --annotate              # Labeled screenshot + legend
  moose screenshot --annotate ./page.png   # Save annotated screenshot
  moose screenshot --annotate --json       # JSON output with annotations
  moose screenshot --screenshot-dir ./shots # Save to custom directory
  moose screenshot --screenshot-format jpeg --screenshot-quality 80
"##
        }
        "pdf" => {
            r##"
moose pdf - Save page as PDF

Usage: moose pdf <path>
...
Examples:
  moose pdf ./page.pdf
  moose pdf ~/Documents/report.pdf
"##
        }
        ...
        "snapshot" => {
            r##"
moose snapshot - Get accessibility tree snapshot

Usage: moose snapshot [options]
...
Examples:
  moose snapshot
  moose snapshot -i
  moose snapshot --compact --depth 5
  moose snapshot -s "#main-content"
"##
        }
        ...
        "eval" => {
            r##"
moose eval - Execute JavaScript

Usage: moose eval [options] <script>
...
Examples:
  ...
  moose eval "window.location.href"
  moose eval "document.querySelectorAll('a').length"
  moose eval -b "ZG9jdW1lbnQudGl0bGU="

  # Read from stdin with heredoc
  cat <<'EOF' | moose eval --stdin
...
"##
        ...
        "inspect" => {
            r##"
moose inspect - Open Chrome DevTools for the active page
...
existing CDP connection, so both DevTools and moose commands work
simultaneously.
...
Examples:
  moose open example.com
  moose inspect          # opens DevTools in your browser
  moose click "Submit"   # commands still work while DevTools is open
"##
        }
        ...
        "get" => {
            r##"
moose get - Retrieve information from elements or page

Usage: moose get <subcommand> [args]
...
Examples:
  moose get text @e1
  moose get html "#content"
  moose get value "#email-input"
  moose get attr "#link" href
  moose get title
  moose get url
  moose get count "li.item"
  moose get box "#header"
  moose get styles "button"
  moose get styles @e1
"##
        }
        ...
        "is" => {
            r##"
moose is - Check element state

Usage: moose is <subcommand> <selector>
...
Examples:
  moose is visible "#modal"
  moose is enabled "#submit-btn"
  moose is checked "#agree-checkbox"
"##
        }
        ...
        "find" => {
            r##"
moose find - Find and interact with elements by locator

Usage: moose find <locator> <value> [action] [text]
...
Examples:
  moose find role button click --name Submit
  moose find text "Sign In" click
  moose find label "Email" fill "user@example.com"
  moose find placeholder "Search..." type "query"
  moose find testid "login-form" click
  moose find first "li.item" click
  moose find nth 2 ".card" hover
"##
        }
        ...
        "mouse" => {
            r##"
moose mouse - Low-level mouse operations

Usage: moose mouse <subcommand> [args]
...
Examples:
  moose mouse move 100 200
  moose mouse down
  moose mouse up
  moose mouse down right
  moose mouse wheel 100
  moose mouse wheel -50 0
"##
        }
        ...
        "set" => {
            r##"
moose set - Configure browser settings

Usage: moose set <setting> [args]
...
Examples:
  moose set viewport 1920 1080
  moose set viewport 1920 1080 2    # 2x retina
  moose set device "iPhone 12"
  moose set geo 37.7749 -122.4194
  moose set offline on
  moose set headers '{"X-Custom": "value"}'
  moose set credentials admin secret123
  moose set media dark
  moose set media light reduced-motion
"##
        }
        ...
        "network" => {
            r##"
moose network - Network interception and monitoring

Usage: moose network <subcommand> [args]
...
Examples:
  moose network route "**/api/*" --abort
  moose network route "**/data.json" --body '{"mock": true}'
  moose network unroute
  moose network requests
  moose network requests --filter "api"
  moose network requests --type xhr,fetch
  moose network requests --method POST --status 2xx
  moose network requests --clear
  moose network request 1234.5
  moose network har start
  moose network har stop ./capture.har
"##
        }
        ...
        "storage" => {
            r##"
moose storage - Manage web storage

Usage: moose storage <type> [operation] [key] [value]
...
Examples:
  moose storage local
  moose storage local get authToken
  moose storage local set theme "dark"
  moose storage local clear
  moose storage session get userId
"##
        }
        ...
        "cookies" => {
            r##"
moose cookies - Manage browser cookies

Usage: moose cookies [operation] [args]
...
Examples:
  ...
  moose cookies set session_id "abc123"

  # Set cookie for a URL before loading it (useful for authentication)
  moose cookies set session_id "abc123" --url https://app.example.com

  # Set secure, httpOnly cookie with domain and path
  moose cookies set auth_token "xyz789" --domain example.com --path /api --httpOnly --secure

  # Set cookie with SameSite policy
  moose cookies set tracking_consent "yes" --sameSite Strict

  # Set cookie with expiration (Unix timestamp)
  moose cookies set temp_token "temp123" --expires 1735689600
...
"##
        ...
        "tab" => {
            r##"
moose tab - Manage browser tabs

Usage: moose tab [operation] [args]
...
Examples:
  ...
  moose tab list
  moose tab new
  moose tab new https://example.com
  moose tab 2
  moose tab close
  moose tab close 1
"##
        ...
        "window" => {
            r##"
moose window - Manage browser windows

Usage: moose window <operation>
...
"##
        ...
        "frame" => {
            r##"
moose frame - Switch frame context

Usage: moose frame <selector|main>
...
Examples:
  moose frame "#embed-iframe"
  moose frame "iframe[name='content']"
  moose frame main
"##
        }
        ...
        "auth" => {
            r##"
moose auth - Manage authentication profiles

Usage: moose auth <subcommand> [args]
...
Examples:
  echo "pass" | moose auth save github --url https://github.com/login --username user --password-stdin
  moose auth save github --url https://github.com/login --username user --password pass
  moose auth login github
  moose auth list
  moose auth show github
  moose auth delete github
"##
        }
        ...
        "confirm" | "deny" => {
            r##"
moose confirm/deny - Approve or deny pending actions

Usage:
  moose confirm <confirmation-id>
  moose deny <confirmation-id>
...
Examples:
  moose confirm c_8f3a1234
  moose deny c_8f3a1234
"##
        }
        ...
        "dialog" => {
            r##"
moose dialog - Handle browser dialogs

Usage: moose dialog <accept|dismiss|status> [text]
...
Examples:
  moose dialog accept
  moose dialog accept "my input"
  moose dialog dismiss
  moose dialog status
"##
        }
        ...
        "trace" => {
            r##"
moose trace - Record execution trace

Usage: moose trace <operation> [path]
...
Examples:
  moose trace start
  moose trace start ./my-trace
  moose trace stop
  moose trace stop ./debug-trace.zip
"##
        }
        ...
        "profiler" => {
            r##"
moose profiler - Record Chrome DevTools performance profile

Usage: moose profiler <operation> [options]
...
Examples:
  # Basic profiling
  moose profiler start
  moose navigate https://example.com
  moose click "#button"
  moose profiler stop ./trace.json

  # With custom categories
  moose profiler start --categories "devtools.timeline,v8.execute,blink.user_timing"
  moose profiler stop ./custom-trace.json
...
"##
        ...
        "record" => {
            r##"
moose record - Record browser session to video

Usage: moose record start <path.webm> [url]
       moose record stop
       moose record restart <path.webm> [url]
...
Examples:
  ...
  moose open https://app.example.com/dashboard
  moose snapshot -i            # Explore and plan
  moose record start ./demo.webm
  moose click @e3              # Execute planned actions
  moose record stop

  # Or specify a different URL
  moose record start ./demo.webm https://example.com

  # Restart recording with a new file (stops previous, starts new)
  moose record restart ./take2.webm
"##
        }
        ...
        "console" => {
            r##"
moose console - View console logs

Usage: moose console [--clear]
...
"##
        ...
        "errors" => {
            r##"
moose errors - View page errors

Usage: moose errors [--clear]
...
"##
        ...
        "highlight" => {
            r##"
moose highlight - Highlight an element

Usage: moose highlight <selector>
...
Examples:
  moose highlight "#target-element"
  moose highlight @e5
"##
        }
        ...
        "clipboard" => {
            r##"
moose clipboard - Read and write clipboard

Usage: moose clipboard <operation> [text]
...
Examples:
  moose clipboard read
  moose clipboard write "Hello, World!"
  moose clipboard copy
  moose clipboard paste
"##
        }
        ...
        "state" => {
            r##"
moose state - Manage browser state

Usage: moose state <operation> [args]
...
Automatic State Persistence:
  ...
  moose --session-name myapp open https://example.com
  Or set MOOSE_SESSION_NAME environment variable.

State Encryption:
  Set MOOSE_ENCRYPTION_KEY (64-char hex) for AES-256-GCM encryption.
...
Examples:
  moose state save ./auth-state.json
  moose state load ./auth-state.json
  moose state list
  moose state show myapp-default.json
  moose state rename old-name new-name
  moose state clear --all
  moose state clean --older-than 7
"##
        }
        ...
        "session" => {
            r##"
moose session - Manage sessions

Usage: moose session [operation]
...
Environment:
  MOOSE_SESSION    Default session name
...
Examples:
  moose session
  moose session list
  moose --session test open example.com
"##
        }
        ...
        "install" => {
            r##"
moose install - Install browser binaries

Usage: moose install [--with-deps]
...
"##
        ...
        "upgrade" => {
            r##"
moose upgrade - Upgrade to the latest version
...
"##
        ...
        "connect" => {
            r##"
moose connect - Connect to browser via CDP

Usage: moose connect <port|url>
...
Examples:
  ...
  moose connect "ws://localhost:9222/devtools/browser/abc123"

  # Connect to remote browser service
  moose connect "wss://browser-service.example.com/cdp?token=xyz"
...
"##
        ...
        "stream" => {
            r##"
moose stream - Manage live WebSocket browser streaming

Usage:
  moose stream enable [--port <port>]
  moose stream disable
  moose stream status

Enables or disables the session-scoped WebSocket stream server without restarting
an already-running daemon. If --port is omitted, moose binds an
...
Notes:
  ...
  - MOOSE_STREAM_PORT only affects daemon startup; use 'stream enable'
...
"##
        ...
        "tap" => {
            r##"
moose tap - Tap an element (touch gesture)

Usage: moose tap <selector>
...
Examples:
  moose tap "#submit-button"
  moose tap @e1
  moose -p ios tap "button:has-text('Sign In')"
"##
        }
        "swipe" => {
            r##"
moose swipe - Swipe gesture (iOS)

Usage: moose swipe <direction> [distance]
...
Examples:
  moose -p ios swipe up
  moose -p ios swipe down 500
  moose -p ios swipe left
"##
        }
        "device" => {
            r##"
moose device - Manage iOS simulators

Usage: moose device <subcommand>
...
Examples:
  moose device list
  moose -p ios device list
"##
        }

        "diff" => {
            r##"
moose diff - Compare page states
...
Snapshot Diff:
  ...
  Usage: moose diff snapshot [options]
...
Screenshot Diff:
  ...
  Usage: moose diff screenshot --baseline <file> [options]
...
URL Diff:
  ...
  Usage: moose diff url <url1> <url2> [options]
...
Examples:
  moose diff snapshot
  moose diff snapshot --baseline before.txt
  moose diff screenshot --baseline before.png
  moose diff screenshot --baseline before.png --output diff.png --threshold 0.2
  moose diff url https://staging.example.com https://prod.example.com
  moose diff url https://v1.example.com https://v2.example.com --screenshot
"##
        }

        "batch" => {
            r##"
moose batch - Execute multiple commands from stdin

Usage: echo '<json>' | moose batch [options]
...
Examples:
  echo '[["open", "https://example.com"], ["snapshot"]]' | moose batch
  echo '[["open", "https://example.com"], ["get", "title"]]' | moose batch --json
  moose batch --bail < commands.json
"##
        }
...
pub fn print_help() {
    println!(
        r#"
moose - fast browser automation CLI for AI agents

Usage: moose <command> [args] [options]
...
Get Info:  moose get <what> [selector]
  text, html, value, attr <name>, title, url, count, box, styles, cdp-url

Check State:  moose is <what> <selector>
  visible, enabled, checked

Find Elements:  moose find <locator> <value> <action> [text]
  role, text, label, placeholder, alt, title, testid, first, last, nth

Mouse:  moose mouse <action> [args]
  move <x> <y>, down [btn], up [btn], wheel <dy> [dx]

Browser Settings:  moose set <setting> [value]
  viewport <w> <h>, device <name>, geo <lat> <lng>
  offline [on|off], headers <json>, credentials <user> <pass>
  media [dark|light] [reduced-motion]

Network:  moose network <action>
...
Authentication:
  ...
  --auto-connect             Connect to a running Chrome to reuse its auth state
                             Tip: moose --auto-connect state save ./auth.json
  --headers <json>           HTTP headers scoped to URL's origin (e.g., Authorization bearer token)

Options:
  --session <name>           Isolated session (or MOOSE_SESSION env)
  --executable-path <path>   Custom browser executable (or MOOSE_EXECUTABLE_PATH)
  --extension <path>         Load browser extensions (repeatable)
  --args <args>              Browser launch args, comma or newline separated (or MOOSE_ARGS)
                             e.g., --args "--no-sandbox,--disable-blink-features=AutomationControlled"
  --user-agent <ua>          Custom User-Agent (or MOOSE_USER_AGENT)
  --proxy <server>           Proxy server URL (or MOOSE_PROXY, HTTP_PROXY, HTTPS_PROXY, ALL_PROXY)
                             Supports authenticated proxies: --proxy "http://user:pass@127.0.0.1:7890"
  --proxy-bypass <hosts>     Bypass proxy for these hosts (or MOOSE_PROXY_BYPASS, NO_PROXY)
  ...
  --screenshot-dir <path>    Default screenshot output directory (or MOOSE_SCREENSHOT_DIR)
  --screenshot-quality <n>   JPEG quality 0-100; ignored for PNG (or MOOSE_SCREENSHOT_QUALITY)
  --screenshot-format <fmt>  Screenshot format: png, jpeg (or MOOSE_SCREENSHOT_FORMAT)
  --headed                   Show browser window (not headless) (or MOOSE_HEADED env)
  --cdp <port>               Connect via CDP (Chrome DevTools Protocol)
  --color-scheme <scheme>    Color scheme: dark, light, no-preference (or MOOSE_COLOR_SCHEME)
  --download-path <path>     Default download directory (or MOOSE_DOWNLOAD_PATH)
  --content-boundaries       Wrap page output in boundary markers (or MOOSE_CONTENT_BOUNDARIES)
  --max-output <chars>       Truncate page output to N chars (or MOOSE_MAX_OUTPUT)
  --allowed-domains <list>   Restrict navigation domains (or MOOSE_ALLOWED_DOMAINS)
  --action-policy <path>     Action policy JSON file (or MOOSE_ACTION_POLICY)
  --confirm-actions <list>   Categories requiring confirmation (or MOOSE_CONFIRM_ACTIONS)
  --confirm-interactive      Interactive confirmation prompts; auto-denies if stdin is not a TTY (or MOOSE_CONFIRM_INTERACTIVE)
  --engine <name>            Browser engine: chrome (default), lightpanda (or MOOSE_ENGINE)
  --config <path>            Use a custom config file (or MOOSE_CONFIG env)
  --debug                    Debug output
  --version, -V              Show version

Configuration:
  moose looks for moose.json in these locations (lowest to highest priority):
    1. ~/.moose/config.json      User-level defaults
    2. ./moose.json              Project-level overrides
    3. Environment variables             Override config file values
    4. CLI flags                         Override everything

  Use --config <path> to load a specific config file instead of the defaults.
  If --config points to a missing or invalid file, moose exits with an error.
...
Environment:
  MOOSE_CONFIG           Path to config file (or use --config)
  MOOSE_SESSION          Session name (default: "default")
  MOOSE_SESSION_NAME     Auto-save/restore state persistence name
  MOOSE_ENCRYPTION_KEY   64-char hex key for AES-256-GCM state encryption
  MOOSE_STATE_EXPIRE_DAYS Auto-delete states older than N days (default: 30)
  MOOSE_EXECUTABLE_PATH  Custom browser executable path
  MOOSE_EXTENSIONS       Comma-separated browser extension paths
  MOOSE_HEADED           Show browser window (not headless)
  MOOSE_JSON             JSON output
  MOOSE_ANNOTATE         Annotated screenshot with numbered labels and legend
  MOOSE_DEBUG            Debug output
  MOOSE_IGNORE_HTTPS_ERRORS Ignore HTTPS certificate errors
  MOOSE_PROVIDER         Browser provider (ios, browserbase, kernel, browseruse, browserless)
  MOOSE_AUTO_CONNECT     Auto-discover and connect to running Chrome
  MOOSE_ALLOW_FILE_ACCESS Allow file:// URLs to access local files
  MOOSE_COLOR_SCHEME     Color scheme preference (dark, light, no-preference)
  MOOSE_DOWNLOAD_PATH    Default download directory for browser downloads
  MOOSE_DEFAULT_TIMEOUT  Default action timeout in ms (default: 25000)
  MOOSE_SESSION_NAME     Auto-save/load state persistence name
  MOOSE_STATE_EXPIRE_DAYS Auto-delete saved states older than N days (default: 30)
  MOOSE_ENCRYPTION_KEY   64-char hex key for AES-256-GCM session encryption
  MOOSE_STREAM_PORT      Enable WebSocket streaming on port (e.g., 9223)
  MOOSE_IDLE_TIMEOUT_MS  Auto-shutdown daemon after N ms of inactivity (disabled by default)
  MOOSE_IOS_DEVICE       Default iOS device name
  MOOSE_IOS_UDID         Default iOS device UDID
  MOOSE_CONTENT_BOUNDARIES Wrap page output in boundary markers
  MOOSE_MAX_OUTPUT       Max characters for page output
  MOOSE_ALLOWED_DOMAINS  Comma-separated allowed domain patterns
  MOOSE_ACTION_POLICY    Path to action policy JSON file
  MOOSE_CONFIRM_ACTIONS  Action categories requiring confirmation
  MOOSE_CONFIRM_INTERACTIVE Enable interactive confirmation prompts
  MOOSE_ENGINE           Browser engine: chrome (default), lightpanda
  HTTP_PROXY / HTTPS_PROXY       Standard proxy env vars (fallback if MOOSE_PROXY not set)
  ALL_PROXY                      SOCKS proxy (fallback for proxy)
  NO_PROXY                       Bypass proxy for hosts (fallback for proxy-bypass)
  MOOSE_SCREENSHOT_DIR   Default screenshot output directory
  MOOSE_SCREENSHOT_QUALITY JPEG quality 0-100
  MOOSE_SCREENSHOT_FORMAT Screenshot format: png, jpeg

Install:
  npm install -g moose           # npm
  brew install moose             # Homebrew
  cargo install moose            # Cargo
  moose install                  # Download Chrome (first time)

Examples:
  moose open example.com
  moose snapshot -i              # Interactive elements only
  moose click @e2                # Click by ref from snapshot
  moose fill @e3 "test@example.com"
  moose find role button click --name Submit
  moose get text @e1
  moose screenshot --full
  moose screenshot --annotate    # Labeled screenshot for vision models
  moose wait --load networkidle  # Wait for slow pages to load
  moose --cdp 9222 snapshot      # Connect via CDP port
  moose --auto-connect snapshot  # Auto-discover running Chrome
  moose stream enable            # Start runtime streaming on an auto-selected port
  moose stream status            # Inspect runtime streaming state
  moose --color-scheme dark open example.com  # Dark mode
  moose --profile ~/.myapp open example.com    # Persistent profile
  moose --session-name myapp open example.com  # Auto-save/restore state

Command Chaining:
  Chain commands with && in a single shell call (browser persists via daemon):

  moose open example.com && moose wait --load networkidle && moose snapshot -i
  moose fill @e1 "user@example.com" && moose fill @e2 "pass" && moose click @e3
  moose open example.com && moose wait --load networkidle && moose screenshot page.png

iOS Simulator (requires Xcode and Appium):
  moose -p ios open example.com                    # Use default iPhone
  moose -p ios --device "iPhone 15 Pro" open url   # Specific device
  moose -p ios device list                         # List simulators
  moose -p ios swipe up                            # Swipe gesture
  moose -p ios tap @e1                             # Touch element
"#
    );
}
...
pub fn print_version() {
    println!("moose {}", env!("CARGO_PKG_VERSION"));
}
