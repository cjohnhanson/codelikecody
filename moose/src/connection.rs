...
/// Priority: MOOSE_SOCKET_DIR > XDG_RUNTIME_DIR > ~/.moose > tmpdir
pub fn get_socket_dir() -> PathBuf {
    // 1. Explicit override (ignore empty string)
    if let Ok(dir) = env::var("MOOSE_SOCKET_DIR") {
        if !dir.is_empty() {
    ...
    if let Ok(runtime_dir) = env::var("XDG_RUNTIME_DIR") {
        if !runtime_dir.is_empty() {
            return PathBuf::from(runtime_dir).join("moose");
        }
    }

    // 3. Home directory fallback (like Docker Desktop's ~/.docker/run/)
    if let Some(home) = dirs::home_dir() {
        return home.join(".moose");
    }

    // 4. Last resort: temp dir
    env::temp_dir().join("moose")
}
...
fn apply_daemon_env(cmd: &mut Command, session: &str, opts: &DaemonOptions) {
    cmd.env("MOOSE_DAEMON", "1")
        .env("MOOSE_SESSION", session);

    if opts.headed {
        cmd.env("MOOSE_HEADED", "1");
    }
    if opts.debug {
        cmd.env("MOOSE_DEBUG", "1");
    }
    if let Some(path) = opts.executable_path {
        cmd.env("MOOSE_EXECUTABLE_PATH", path);
    }
    if !opts.extensions.is_empty() {
        cmd.env("MOOSE_EXTENSIONS", opts.extensions.join(","));
    }
    if let Some(a) = opts.args {
        cmd.env("MOOSE_ARGS", a);
    }
    if let Some(ua) = opts.user_agent {
        cmd.env("MOOSE_USER_AGENT", ua);
    }
    if let Some(p) = opts.proxy {
        cmd.env("MOOSE_PROXY", p);
    }
    if let Some(pb) = opts.proxy_bypass {
        cmd.env("MOOSE_PROXY_BYPASS", pb);
    }
    if let Some(pu) = opts.proxy_username {
        cmd.env("MOOSE_PROXY_USERNAME", pu);
    }
    if let Some(pp) = opts.proxy_password {
        cmd.env("MOOSE_PROXY_PASSWORD", pp);
    }
    if opts.ignore_https_errors {
        cmd.env("MOOSE_IGNORE_HTTPS_ERRORS", "1");
    }
    if opts.allow_file_access {
        cmd.env("MOOSE_ALLOW_FILE_ACCESS", "1");
    }
    if let Some(prof) = opts.profile {
        cmd.env("MOOSE_PROFILE", prof);
    }
    if let Some(st) = opts.state {
        cmd.env("MOOSE_STATE", st);
    }
    if let Some(p) = opts.provider {
        cmd.env("MOOSE_PROVIDER", p);
    }
    if let Some(d) = opts.device {
        cmd.env("MOOSE_IOS_DEVICE", d);
    }
    if let Some(sn) = opts.session_name {
        cmd.env("MOOSE_SESSION_NAME", sn);
    }
    if let Some(dp) = opts.download_path {
        cmd.env("MOOSE_DOWNLOAD_PATH", dp);
    }
    if let Some(ad) = opts.allowed_domains {
        cmd.env("MOOSE_ALLOWED_DOMAINS", ad.join(","));
    }
    if let Some(ap) = opts.action_policy {
        cmd.env("MOOSE_ACTION_POLICY", ap);
    }
    if let Some(ca) = opts.confirm_actions {
        cmd.env("MOOSE_CONFIRM_ACTIONS", ca);
    }
    if let Some(engine) = opts.engine {
        cmd.env("MOOSE_ENGINE", engine);
    }
    if opts.auto_connect {
        cmd.env("MOOSE_AUTO_CONNECT", "1");
    }
    if let Some(idle) = opts.idle_timeout {
        cmd.env("MOOSE_IDLE_TIMEOUT_MS", idle);
    }
    if let Some(cdp) = opts.cdp {
        cmd.env("MOOSE_CDP", cdp);
    }
}
        ...
        if path_len > 103 {
            return Err(format!(
                "Session name '{}' is too long. Socket path would be {} bytes (max 103).\n\
                 Use a shorter session name or set MOOSE_SOCKET_DIR to a shorter path.",
                session, path_len
            ));
        }
    }
...
mod tests {
    ...
    fn test_get_socket_dir_xdg_runtime() {
        ...
        assert_eq!(
            get_socket_dir(),
            PathBuf::from("/run/user/1000/moose")
        );
    }
