...
const CONFIG_DIR: &str = ".moose";
const CONFIG_FILENAME: &str = "config.json";
const PROJECT_CONFIG_FILENAME: &str = "moose.json";
...
pub fn load_config(args: &[String]) -> Result<Config, String> {
    let explicit = extract_config_path(args)
        .map(|p| ("--config", p))
        .or_else(|| {
            env::var("MOOSE_CONFIG")
                .ok()
                .map(|p| ("MOOSE_CONFIG", Some(p)))
        });
...
pub struct Flags {
    ...
    pub screenshot_quality: Option<u32>,
    pub screenshot_format: Option<String>,
    pub idle_timeout: Option<String>, // Canonical milliseconds string for MOOSE_IDLE_TIMEOUT_MS
...
pub fn parse_flags(args: &[String]) -> Flags {
    ...
    let extensions_env = env::var("MOOSE_EXTENSIONS")
        .ok()
        .map(|s| {
            s.split(',')
    ...
    let mut flags = Flags {
        json: env_var_is_truthy("MOOSE_JSON") || config.json.unwrap_or(false),
        headed: env_var_is_truthy("MOOSE_HEADED") || config.headed.unwrap_or(false),
        debug: env_var_is_truthy("MOOSE_DEBUG") || config.debug.unwrap_or(false),
        session: env::var("MOOSE_SESSION")
            .ok()
            .or(config.session)
            .unwrap_or_else(|| "default".to_string()),
        headers: config.headers,
        executable_path: env::var("MOOSE_EXECUTABLE_PATH")
            .ok()
            .or(config.executable_path),
        cdp: config.cdp,
        extensions,
        profile: env::var("MOOSE_PROFILE").ok().or(config.profile),
        state: env::var("MOOSE_STATE").ok().or(config.state),
        proxy: env::var("MOOSE_PROXY")
            .ok()
            .or(config.proxy)
            ...
            .or_else(|| env::var("all_proxy").ok()),
        proxy_bypass: env::var("MOOSE_PROXY_BYPASS")
            .ok()
            .or(config.proxy_bypass)
            .or_else(|| env::var("NO_PROXY").ok())
            .or_else(|| env::var("no_proxy").ok()),
        args: env::var("MOOSE_ARGS").ok().or(config.args),
        user_agent: env::var("MOOSE_USER_AGENT")
            .ok()
            .or(config.user_agent),
        provider: env::var("MOOSE_PROVIDER").ok().or(config.provider),
        ignore_https_errors: env_var_is_truthy("MOOSE_IGNORE_HTTPS_ERRORS")
            || config.ignore_https_errors.unwrap_or(false),
        allow_file_access: env_var_is_truthy("MOOSE_ALLOW_FILE_ACCESS")
            || config.allow_file_access.unwrap_or(false),
        device: env::var("MOOSE_IOS_DEVICE").ok().or(config.device),
        auto_connect: env_var_is_truthy("MOOSE_AUTO_CONNECT")
            || config.auto_connect.unwrap_or(false),
        session_name: env::var("MOOSE_SESSION_NAME")
            .ok()
            .or(config.session_name),
        annotate: env_var_is_truthy("MOOSE_ANNOTATE") || config.annotate.unwrap_or(false),
        color_scheme: env::var("MOOSE_COLOR_SCHEME")
            .ok()
            .or(config.color_scheme),
        download_path: env::var("MOOSE_DOWNLOAD_PATH")
            .ok()
            .or(config.download_path),
        content_boundaries: env_var_is_truthy("MOOSE_CONTENT_BOUNDARIES")
            || config.content_boundaries.unwrap_or(false),
        max_output: env::var("MOOSE_MAX_OUTPUT")
            .ok()
            .and_then(|s| s.parse().ok())
            .or(config.max_output),
        allowed_domains: env::var("MOOSE_ALLOWED_DOMAINS")
            .ok()
            .map(|s| {
                s.split(',')
        ...
        action_policy: env::var("MOOSE_ACTION_POLICY")
            .ok()
            .or(config.action_policy),
        confirm_actions: env::var("MOOSE_CONFIRM_ACTIONS")
            .ok()
            .or(config.confirm_actions),
        confirm_interactive: env_var_is_truthy("MOOSE_CONFIRM_INTERACTIVE")
            || config.confirm_interactive.unwrap_or(false),
        engine: env::var("MOOSE_ENGINE").ok().or(config.engine),
        screenshot_dir: env::var("MOOSE_SCREENSHOT_DIR")
            .ok()
            .or(config.screenshot_dir),
        screenshot_quality: env::var("MOOSE_SCREENSHOT_QUALITY")
            .ok()
            .and_then(|s| s.parse().ok())
            .or(config.screenshot_quality),
        screenshot_format: env::var("MOOSE_SCREENSHOT_FORMAT")
            .ok()
            .or(config.screenshot_format)
            .filter(|s| s == "png" || s == "jpeg"),
        idle_timeout: parse_idle_timeout_value(
            env::var("MOOSE_IDLE_TIMEOUT_MS").ok(),
            "MOOSE_IDLE_TIMEOUT_MS",
        )
...
mod tests {
    ...
    fn test_load_config_missing_file_returns_none() {
        let result = read_config_file(&PathBuf::from("/nonexistent/moose.json"));
