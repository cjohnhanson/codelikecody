...
const NPM_REGISTRY_URL: &str = "https://registry.npmjs.org/moose/latest";
...
fn detect_install_method() -> InstallMethod {
    if let Ok(exe) = std::env::current_exe() {
        ...
        if path_str.contains("/Cellar/moose/")
        ...
        if path_str.contains("node_modules/moose")
            || path_str.contains("node_modules\\moose")
        {
    ...
    {
        if command_succeeds("brew", &["list", "moose"]) {
            return InstallMethod::Homebrew;
        }
    }
    ...
    if command_output_contains(
        "pnpm",
        &["list", "-g", "moose", "--depth=0"],
        "moose",
    ) {
        return InstallMethod::Pnpm;
    }

    if command_output_contains("yarn", &["global", "list", "--depth=0"], "moose") {
        return InstallMethod::Yarn;
    }

    if command_output_contains("bun", &["pm", "ls", "-g"], "moose") {
        return InstallMethod::Bun;
    }

    if command_succeeds("npm", &["list", "-g", "moose", "--depth=0"]) {
        return InstallMethod::Npm;
    }
...
fn run_upgrade_command(method: &InstallMethod) -> bool {
    let (cmd, args, display): (&str, &[&str], &str) = match method {
        InstallMethod::Npm => (
            "npm",
            &["install", "-g", "moose@latest"],
            "npm install -g moose@latest",
        ),
        InstallMethod::Pnpm => (
            "pnpm",
            &["add", "-g", "moose@latest"],
            "pnpm add -g moose@latest",
        ),
        ...
        InstallMethod::Yarn => (
            "yarn",
            &["global", "add", "moose@latest"],
            "yarn global add moose@latest",
        ),
        InstallMethod::Bun => (
            "bun",
            &["install", "-g", "moose@latest"],
            "bun install -g moose@latest",
        ),
...
pub fn run_upgrade() {
    ...
    if !latest.is_empty() && current == latest.as_str() {
        println!(
            "{} moose is already at the latest version (v{})",
    ...
    if matches!(method, InstallMethod::Unknown) {
        ...
        eprintln!("    npm install -g moose@latest       # npm");
        eprintln!("    pnpm add -g moose@latest          # pnpm");
        eprintln!("    yarn global add moose@latest       # yarn");
        eprintln!("    bun install -g moose@latest        # bun");
        eprintln!("    brew upgrade moose                 # Homebrew");
        eprintln!("    cargo install moose --force        # Cargo");
        exit(1);
    }
    ...
    if !latest.is_empty() {
        ...
        println!(
            "{}",
            color::cyan(&format!("Upgrading moose (v{})...", current))
        );
    }
