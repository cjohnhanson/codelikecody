...
fn build_chrome_args(options: &LaunchOptions) -> Result<ChromeArgs, String> {
    ...
    let (user_data_dir, temp_user_data_dir) = if let Some(ref profile) = options.profile {
        ...
        let dir =
            std::env::temp_dir().join(format!("moose-chrome-{}", uuid::Uuid::new_v4()));
...
pub fn launch_chrome(options: &LaunchOptions) -> Result<ChromeProcess, String> {
    let chrome_path = match &options.executable_path {
        ...
        None => {
            find_chrome().ok_or("Chrome not found. Run `moose install` to download Chrome, or use --executable-path.")?
        }
    };
...
pub fn find_chrome() -> Option<PathBuf> {
    // 1. Check Chrome downloaded by `moose install`
