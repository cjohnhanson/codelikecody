    ...
    pub fn new() -> Self {
        Self {
            ...
            domain_filter: Arc::new(RwLock::new(
                env::var("MOOSE_ALLOWED_DOMAINS")
                    .ok()
                    .filter(|s| !s.is_empty())
                    .map(|s| DomainFilter::new(&s)),
            )),
            event_tracker: EventTracker::new(),
            session_name: env::var("MOOSE_SESSION_NAME").ok(),
            session_id: env::var("MOOSE_SESSION").unwrap_or_else(|_| "default".to_string()),
...
pub async fn execute_command(cmd: &Value, state: &mut DaemonState) -> Value {
    ...
    // Check MOOSE_CONFIRM_ACTIONS (category-based, independent of policy file)
    ...
    let result = match action {
        ...
        "animation_list" => super::animation::handle_animation_list(cmd, state).await,
        "animation_pause" => super::animation::handle_animation_pause(cmd, state).await,
        "animation_resume" => super::animation::handle_animation_resume(cmd, state).await,
        "animation_slow" => super::animation::handle_animation_slow(cmd, state).await,
        "animation_seek" => super::animation::handle_animation_seek(cmd, state).await,
        "wait_animation" => super::animation::handle_wait_animation(cmd, state).await,
...
async fn auto_launch(state: &mut DaemonState) -> Result<(), String> {
    ...
    let engine = env::var("MOOSE_ENGINE").ok();
    ...
    if let Ok(cdp) = env::var("MOOSE_CDP") {
    ...
    if env::var("MOOSE_AUTO_CONNECT").is_ok() {
        state.reset_input_state();
...
fn launch_options_from_env() -> LaunchOptions {
    let headed = env::var("MOOSE_HEADED")
        .map(|v| v == "1" || v == "true")
        .unwrap_or(false);

    let extensions: Option<Vec<String>> = env::var("MOOSE_EXTENSIONS").ok().map(|v| {
        v.split([',', '\n'])
    ...
    LaunchOptions {
        headless: !headed,
        executable_path: env::var("MOOSE_EXECUTABLE_PATH").ok(),
        proxy: env::var("MOOSE_PROXY").ok(),
        proxy_bypass: env::var("MOOSE_PROXY_BYPASS").ok(),
        proxy_username: env::var("MOOSE_PROXY_USERNAME").ok(),
        proxy_password: env::var("MOOSE_PROXY_PASSWORD").ok(),
        profile: env::var("MOOSE_PROFILE").ok(),
        allow_file_access: env::var("MOOSE_ALLOW_FILE_ACCESS")
            .map(|v| v == "1" || v == "true")
            .unwrap_or(false),
        args: env::var("MOOSE_ARGS")
            .map(|v| {
                v.split([',', '\n'])
                    .map(|s| s.trim().to_string())
                    .filter(|s| !s.is_empty())
            ...
            .unwrap_or_default(),
        extensions,
        storage_state: env::var("MOOSE_STATE").ok(),
        user_agent: env::var("MOOSE_USER_AGENT").ok(),
        ignore_https_errors: env::var("MOOSE_IGNORE_HTTPS_ERRORS")
            .map(|v| v == "1" || v == "true")
            .unwrap_or(false),
        color_scheme: env::var("MOOSE_COLOR_SCHEME").ok(),
        download_path: env::var("MOOSE_DOWNLOAD_PATH").ok(),
    }
}
...
async fn handle_launch(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let executable_path: Option<String> = cmd
        ...
        .and_then(|v| v.as_str())
        .map(String::from)
        .or_else(|| std::env::var("MOOSE_EXECUTABLE_PATH").ok());
    ...
    let engine = cmd
        ...
        .and_then(|v| v.as_str())
        .map(String::from)
        .or_else(|| env::var("MOOSE_ENGINE").ok());
    ...
    let options = LaunchOptions {
        ...
        executable_path: cmd
            ...
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
            .or_else(|| env::var("MOOSE_EXECUTABLE_PATH").ok()),
        ...
        proxy_username: cmd
            ...
            .and_then(|v| v.as_str())
            .map(String::from)
            .or_else(|| env::var("MOOSE_PROXY_USERNAME").ok()),
        proxy_password: cmd
            .get("proxy")
            .and_then(|v| v.get("password"))
            .and_then(|v| v.as_str())
            .map(String::from)
            .or_else(|| env::var("MOOSE_PROXY_PASSWORD").ok()),
        user_agent: cmd
            .get("userAgent")
            .and_then(|v| v.as_str())
            .map(String::from),
...
async fn handle_getbyrole(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let js = format!(
        r#"(() => {{
            ...
            for (const el of els) {{
                if ({name_match}) {{
                    el.setAttribute('data-moose-located', 'true');
                    return true;
                }}
            }}
            return false;
    ...
    let selector = "[data-moose-located='true']";
    ...
    if let Some(ref browser) = state.browser {
        if browser.active_session_id().is_ok() {
            let _ = browser
                .evaluate(
                    "document.querySelector('[data-moose-located]')?.removeAttribute('data-moose-located')",
                    None,
                )
                .await;
        }
    ...
    let query = match strategy {
        "label" => format!(
            r#"(() => {{
                ...
                if (target) {{ target.setAttribute('data-moose-located', 'true'); return true; }}
                return false;
            }})()"#,
            match_fn = match_fn,
        ),
        "placeholder" => format!(
            r#"(() => {{
                ...
                if (el) {{ el.setAttribute('data-moose-located', 'true'); return true; }}
                return false;
            }})()"#,
...
async fn handle_nth(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let js = format!(
        r#"(() => {{
            ...
            els[idx].setAttribute('data-moose-located', 'true');
            return true;
        }})()"#,
        sel = serde_json::to_string(selector).unwrap_or_default(),
        idx = index,
    ...
    let located = "[data-moose-located='true']";
...
fn get_har_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        home.join(".moose").join("tmp").join("har")
    } else {
        std::env::temp_dir().join("moose").join("har")
    }
}
...
mod tests {
    ...
    async fn test_stream_enable_disable_and_status_without_browser() {
        ...
        guard.set("MOOSE_SESSION", "stream-runtime-session");
    ...
    async fn test_stream_disable_clears_state_when_stream_file_removal_fails() {
        ...
        guard.set("MOOSE_SESSION", "stream-disable-cleanup-session");
    ...
    async fn test_stream_enable_port_conflict_returns_error() {
        ...
        guard.set("MOOSE_SESSION", "stream-port-conflict-session");
    ...
    fn test_launch_options_from_env_headed_flag() {
        ...
        assert!(
            !opts.headless,
            "MOOSE_HEADED=1 should set headless=false"
        );
    }
    ...
    async fn test_handle_har_stop_without_path_uses_default_location() {
        ...
        assert_eq!(har["log"]["creator"]["name"], "moose");
    ...
    async fn test_credentials_roundtrip_via_actions() {
        ...
        let key_var = "MOOSE_ENCRYPTION_KEY";
