...
pub async fn run_daemon(session: &str) {
    ...
    if let Ok(days_str) = env::var("MOOSE_STATE_EXPIRE_DAYS") {
    ...
    if let Ok(port_str) = env::var("MOOSE_STREAM_PORT") {
    ...
    let idle_timeout_ms = env::var("MOOSE_IDLE_TIMEOUT_MS")
        .ok()
...
fn get_daemon_socket_dir() -> PathBuf {
    ...
    if let Ok(xdg) = env::var("XDG_RUNTIME_DIR") {
        if !xdg.is_empty() {
            return PathBuf::from(xdg).join("moose");
        }
    }
