...
/// Confirmation categories parsed from MOOSE_CONFIRM_ACTIONS.
#[derive(Debug, Clone)]
...
impl ConfirmActions {
    pub fn from_env() -> Option<Self> {
        let val = env::var("MOOSE_CONFIRM_ACTIONS").ok()?;
        if val.is_empty() {
            return None;
        }
...
impl ActionPolicy {
    ...
    /// Load policy if MOOSE_ACTION_POLICY env var is set.
    /// Falls back to MOOSE_POLICY for backwards compatibility.
    pub fn load_if_exists() -> Option<Self> {
        let path = env::var("MOOSE_ACTION_POLICY")
            .or_else(|_| env::var("MOOSE_POLICY"))
            .ok()?;
        Self::load(&path).ok()
    }
