...
fn main() {
    ...
    // Native daemon mode: when MOOSE_DAEMON is set, run as the daemon process
    if env::var("MOOSE_DAEMON").is_ok() {
        ...
        let session = env::var("MOOSE_SESSION").unwrap_or_else(|_| "default".to_string());
    ...
    if daemon_result.already_running {
        ...
        if !ignored_flags.is_empty() && !flags.json {
            eprintln!(
                "{} {} ignored: daemon already running. Use 'moose close' first to restart with new options.",
                color::warning_indicator(),
                ignored_flags.join(", ")
            );
        }
            ...
            if flags.confirm_interactive {
                if let Some(data) = &resp.data {
                        ...
                        .unwrap_or(false)
                    {
                        ...
                        eprintln!("[moose] Action requires confirmation:");
