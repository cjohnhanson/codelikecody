...
pub struct ProgressReporter {
    ...
    /// All bars, keyed by path index. Pre-created in order so they display correctly.
...
impl ProgressReporter {
    pub fn new() -> Self {
        if !std::io::stderr().is_terminal() {
            return Self {
                ...
                pending_style: ProgressStyle::default_spinner(),
        ...
        let style = ProgressStyle::with_template("{spinner:.cyan} {msg}")
            .unwrap()
            .tick_chars("⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏");
        let pending_style = ProgressStyle::with_template("{msg:.dim}").unwrap();
    ...
    /// Pre-create all progress bars in order so they maintain stable positions.
    pub fn prepare(&self, paths: &[crate::paths::TestPath], graph: &crate::graph::StateGraph) {
        ...
        for (i, path) in paths.iter().enumerate() {
            ...
            let msg = format!("[{}/{}] {}", i + 1, total, path.display(graph));
            bar.set_message(msg.clone());
            bars.insert(i, (bar, msg));
        }
    }
    ...
    pub fn on_event(&self, event: ProgressEvent) {
        ...
        match event {
            ProgressEvent::PathStarted { index, .. } => {
            ...
            ProgressEvent::PathFinished { index, passed } => {
                if let Some((bar, msg)) = self.bars.lock().unwrap().get(&index) {
